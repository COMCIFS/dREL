...
# Test the autogenerated dREL parser
...
frag_collection["call_suite"] = """
   If(setting == 'triclinic') {
      If( Abs(a-b)<d || Abs(a-c)<d || Abs(b-c)<d )        x = y
      If( Abs(alp-90)<d || Abs(bet-90)<d || Abs(gam-90)<d ) y = z
      }
"""
...
@pytest.fixture
def load_ddlmdic(request,scope="module"):
    cifdic = getattr(request.module,"ddlm_dic")
    return CifDic(cifdic,do_imports='Contents',do_dREL=False)
...
def drel_methods(request,scope="module"):
    ...
    has_meth.sort()
    return [(n,p[n]['_method.expression']) for n in has_meth if p[n]['_method.purpose']=='Evaluation']
...
class TestGrammar(object):
    ...
    def test_subs(self,lark_grammar,pytransformer):
        process_a_phrase("c[0]=1",lark_grammar,pytransformer)
...
class TestExecBlock(object):
    ...
    def test_loop_calcs(self,getexecdata):
        ...
        r = getexecdata['_atom_type.number_in_cell']
        print(r)
        assert r == [12.0,28.0,0.0]  #no H information
...
@pytest.fixture
def valid_transformer(request,scope="module"):
    dd = load_ddlmdic(request,scope)
    cd = load_cifdic(request,scope)
    tt = TreeToPy("cell.atomic_mass","myfunc",
                  cd,
                  True,dd)
    lg = lark_grammar(request,scope)
    return tt,lg

class TestValidMethods(object):
    """Test validation methods"""
    def test_is_known(self,load_ddlmdic,getdata,valid_transformer):
        test_frag = "cell.atomic_mass = Known('_atom_site.label')"
        tt,lg = valid_transformer
        new_text = tt.transform(lg.parse(test_frag))
        print(new_text)
        exec(new_text,globals())
        result = myfunc(getdata,load_cifdic,{},'dummy',load_ddlmdic)
        assert result

    def test_reference(self,load_cifdic,load_ddlmdic,getdata,valid_transformer):
        test_frag = "cell.atomic_mass = Reference('_diffrn.ambient_environment','_name.object_id')"
        tt,lg = valid_transformer
        new_text = tt.transform(lg.parse(test_frag))
        print(new_text)
        exec(new_text,globals())
        result = myfunc(getdata,load_cifdic,{},'dummy',load_ddlmdic)
        assert result == 'ambient_environment'

# Just ignore the use of 'float' to convert a string to a number in
# the following, that is not what we are testing here.
    def test_instance(self,load_ddlmdic,getdata,valid_transformer,load_cifdic):
        test_frag = """
tot = 0.0
atype = Instance('atom_type')
loop i as atype {
    tot = tot + float(i.number_in_cell)
    }
cell.atomic_mass = tot """    
        tt,lg = valid_transformer
        new_text = tt.transform(lg.parse(test_frag))
        print(new_text)
        exec(new_text,globals())
    ...
    def test_packetdata(self,load_ddlmdic,getdata,valid_transformer,load_cifdic):
        """Test that we can access packet data"""
        test_frag = """
tot = 0.0
loop i as atom_type {
    tot = tot + float(packetdata(i,'number_in_cell'))
    }
cell.atomic_mass = tot """    
        tt,lg = valid_transformer
        new_text = tt.transform(lg.parse(test_frag))
    ...
    def test_lookup(self,load_ddlmdic,getdata,valid_transformer,load_cifdic):
        test_frag = """
        q = Instance('atom_site')
cell.atomic_mass = packetdata(lookup(q,{'label':'c4'}),'fract_z')
"""
        tt,lg = valid_transformer
        new_text = tt.transform(lg.parse(test_frag))
        print(new_text)
